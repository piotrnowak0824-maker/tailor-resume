...
3. In each Experience/job entry, produce 4–7 bullets (one sentence per bullet), each a concise storytelling sentence (challenge → action → result). This is a hard requirement: NEVER fewer than 4 bullets per role. The longest company should have 7 bullets, and the others should have 4-7 bullets according to company period length. Aggressively update, rewrite, or ADD new bullets so they reflect the actual duties, skills, or stacks requested in the JD, especially prioritizing skills, tools, or requirements from the current and most recent positions. If the source role has fewer bullets, CREATE additional realistic, JD-aligned bullets.
...
SUMMARY OPENING: The Summary must always align with title from job description.
...
OUTPUT: Return the improved resume as a single JSON object only (no other text, no markdown). Use this exact structure. Preserve all company names, job titles, and dates from the base resume. Use **bold** for technical terms in summary and in experience details as per your bold rules. Order experience by date (most recent first). Include 4-7 bullets per role in details. If you added a Cascade Investment role, include it in experience with its company, title, dates, and details.
...
{"title":"<exact job title from JD only, no company>","summary":"<**bold** for tech terms; if 10+ years exp use only 'more than 10 years'>","skills":{"<CategoryName>":["skill1","skill2",...],...},"experience":[{"title":"<job title>","company":"<company name>","location":"<location or empty string>","start_date":"<start>","end_date":"<end>","details":["<bullet with **bold**>",...]}]}`;
    ...
    if (finishReason === "length") {
      ...
      const concisePrompt = prompt
        .replace(/4–7 bullets per role/g, "3–5 bullets per role")
        .replace(/NEVER fewer than 4 bullets per role/g, "NEVER fewer than 3 bullets per role");
